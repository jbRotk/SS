...
public class DownloadControl {
    ...
    public ArrayList<SongMsg> QueryDownload()//返回所有的
    {
        ArrayList<SongMsg> songMsgs = new ArrayList<SongMsg>();
        SQLiteDatabase db = context.openOrCreateDatabase(DownloadDatabases.DatabaseName,Context.MODE_PRIVATE,null);
        Cursor cursor;
        try {
            cursor = db.query(DownloadDatabases.TableName,null,null,null,null,null,null);
            while (!cursor.isLast())
            {
                cursor.moveToNext();
    ...
    public void InserObject(SongMsg songMsg)
    {
       ...
       // db.insert()
        ...
        db.insert(DownloadDatabases.TableName,null,contentValues);
    }
}
